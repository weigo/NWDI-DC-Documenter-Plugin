			...
			<xsl:element name="head">
				...
				<xsl:element name="link">
					...
					<xsl:attribute name="href"><xsl:value-of
						select="concat($cssLocation, '/report.css')" />
					</xsl:attribute>
				</xsl:element>
			</xsl:element>
			<xsl:element name="body">
				<xsl:attribute name="name"><xsl:text></xsl:text><xsl:value-of
					select="@sc-name" />
								</xsl:attribute>
			</xsl:element>
			<xsl:element name="h3">
					...
					<xsl:for-each select="development-components/development-component">
						...
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								<xsl:element name="a">
									<xsl:attribute name="href"><xsl:value-of
										select="concat(@vendor, '~', translate(@name,'/','~'), '.html')" />
								</xsl:attribute>
			...
			<xsl:when test="$imageFormat = 'svg'">
				<xsl:element name="object">
					<xsl:attribute name="data"><xsl:value-of
						select="$imageLocation" /><xsl:text>/</xsl:text><xsl:value-of
						select="$name" /><xsl:text>.</xsl:text><xsl:value-of
						select="$imageFormat" /></xsl:attribute>
